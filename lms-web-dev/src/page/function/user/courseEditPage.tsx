...
import { Table, Button, Space, Modal, Form, Input, Tag, message, Typography } from 'antd';
...
const CourseEdit: React.FC = () => {
    ...
    const loadCourses = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get(`/api/courses/${lecturerId}`);
            setCourses(res.data);
        } catch (error) {
    ...
    const handleSave = async () => {
        try {
            ...
            if (editingCourse) {
                ...
                await axios.post(`/api/courses`, { ...values, lecturer_id: lecturerId });
    ...
    const showDeleteConfirm = (courseId: number) => {
        confirm({
            ...
            onOk: async () => {
                try {
                    await axios.post(`/api/courses/request-delete`, { id: courseId });
        ...
        <div style={{ padding: '24px', background: '#fff', borderRadius: '8px' }}>
            ...
            >
                <Form form={form} layout="vertical" style={{ marginTop: '20px' }}>
                    ...
                    <Form.Item
                        name="password"
                        label="Mật khẩu lớp học (Để trống nếu muốn vào tự do)"
                        tooltip="Nếu đặt mật khẩu, sinh viên phải nhập đúng mới có thể tham gia lớp."
                    >
                        <Input.Password placeholder="Nhập mật khẩu lớp" />
                    </Form.Item>
